...
class ReverseShellListener:
    ...
    def clear_screen(self):
        """Clear terminal for a fresh menu display."""
...
if __name__ == "__main__":
    try:
        host_input = input("INPUT IP (leave blank for local IP): ").strip()
        host = host_input if host_input else None

        port_input = input("INPUT PORT (leave blank for random): ").strip()
        if port_input:
            try:
                port = int(port_input)
            except ValueError:
                print("[!] Invalid port number, using random free port.")
                port = None
        else:
            port = None
